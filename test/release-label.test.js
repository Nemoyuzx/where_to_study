...
test("all tracked client projects use the stable 0.2.5 release version", () => {
  ...
  assert.equal(tauriMetadata.bundle.android.versionCode, 2005);
  ...
  assert.match(nativeAndroid, /versionCode = 38/);
  assert.match(nativeApple, /MARKETING_VERSION: "0\.2\.5"/);
  assert.match(nativeApple, /CURRENT_PROJECT_VERSION: "66"/);
  assert.match(nativeHarmony, /"versionName": "0\.2\.5"/);
  assert.match(nativeHarmony, /"versionCode": 1002005/);
