...
const appleQueryPicker = text('../native/apple/Sources/Shared/InformationQueryModePicker.swift')
...
test('every graphical platform exposes query as a primary destination between calendar and settings', () => {
  ...
  assert.match(appleQuery, /InformationQueryModePicker\(/)
  assert.match(appleQueryPicker, /UIDevice\.current\.userInterfaceIdiom == \.phone/)
  ...
  assert.match(androidQuery, /SHUTTLE\("班车查询", "班车", "Shuttle", R\.drawable\./)
  assert.match(androidQuery, /IMPORTANT_EVENTS\("重要事件", "事件", "Events", R\.drawable\./)
  assert.match(androidQuery, /GRADES\("成绩查询", "成绩", "Grades", R\.drawable\./)
  assert.match(androidQuery, /EXAMS\("考试查询", "考试安排", "Exams", R\.drawable\./)
  assert.match(androidQuery, /ASSIGNMENTS\("课程作业", "作业", "Tasks", R\.drawable\./)
  assert.match(androidQuery, /smallestScreenWidthDp < 600/)
  assert.match(androidQuery, /usesIconOnlyTabs\(/)
  assert.match(androidQuery, /private fun modeSelector\(\): FrameLayout/)
  ...
  assert.match(harmonyQuery, /SymbolGlyph\(QueryPageContract\.tabSymbol\(tab\)\)/)
  assert.match(harmonyQuery, /\.layoutWeight\(1\)/)
