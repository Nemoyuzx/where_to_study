...
test('every graphical platform exposes query as a primary destination between calendar and settings', () => {
  ...
  assert.match(androidQuery, /SHUTTLE\("班车查询", "班车", "Shuttle"\)/)
  assert.match(androidQuery, /IMPORTANT_EVENTS\("重要事件", "事件", "Events"\)/)
  assert.match(androidQuery, /GRADES\("成绩查询", "成绩", "Grades"\)/)
  assert.match(androidQuery, /EXAMS\("考试查询", "考试安排", "Exams"\)/)
  assert.match(androidQuery, /ASSIGNMENTS\("课程作业", "作业", "Tasks"\)/)
  assert.match(androidQuery, /if \(availableWidthDp < 560\)/)
