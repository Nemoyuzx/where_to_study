...
应用提供可调整尺寸的原生“今日课程”桌面小组件，直接读取应用私有课表缓存，展示日期、星期、教学周、当前或下一节状态、时间、节次、教室与教师。Android 12 及以上按启动器提供的实际尺寸生成布局，旧版按横竖屏尺寸适配；在实际容量和用户课程上限内优先显示今日课程，剩余行补充带“明日”标识的课程，总数最多 6 门。明日课程按明日日期和教学周计算，不参与今日当前或下一节状态。今日无课但明日有课时显示明日课程和今日数量 0，两日都无课时显示“今日无课”。设置预览使用虚构的今日及明日课程，与真实小组件共用行选择和标题逻辑。课表刷新、账号清除、日期或时区变化以及应用更新后会重绘，另有北京时间午夜的非精确定时重绘与系统定期刷新，均不新增网络请求。
