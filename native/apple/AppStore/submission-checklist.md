...
- `preflight all`、macOS/iOS `APP_STORE_BUILD` Release 编译、Apple 逻辑测试、iPhone/iPad 专项 UI 验证和 Universal macOS `0.1.4 (37)` 正式签名归档均已通过；两个主应用归档均核对为 `com.nemoyu.wheretostudy.native.macos`，应用图标统一为 Windows/Tauri 当前使用的绿色日历课桌图标。
- iOS 与 macOS `0.1.4 (37)` 均已收到 App Store Connect 的 `Upload succeeded`；当前等待 Apple 处理。TestFlight 和正式提交选择各平台的最新构建。
...
- App Store Connect 处理完成后，将 iOS/macOS build 37 加入内部 TestFlight 群组并在真机验证登录、课表、空教室、日历导入、通知、深浅色和前后台切换。
