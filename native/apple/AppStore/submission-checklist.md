...
- iOS 与 macOS `0.1.9 (49)` 均已收到 Xcode 的 `Upload succeeded`，上传完成；按发布流程未再打开浏览器复核。TestFlight 和正式提交应分别选择对应平台的最新构建。
...
- App Store Connect 处理完成后，将 iOS 与 macOS build 49 加入内部 TestFlight 群组并在真机验证登录、课表、空教室、日历导入、通知、深浅色和前后台切换。
