...
- macOS `0.2.1 (50)` 的预检、Release 归档、137 项 Apple 逻辑测试均已通过；iOS 同版本的 138 项逻辑测试和 12 项 UI 测试也已回归。两个主应用标识均已核对为 `com.nemoyu.wheretostudy.native.macos`，应用图标统一为 Windows/Tauri 当前使用的绿色日历课桌图标。
- iOS 与 macOS `0.2.1 (50)` 均已收到 Xcode 的 `Upload succeeded` 并进入 App Store Connect processing；TestFlight 和正式提交应分别选择对应平台的最新构建。
...
- App Store Connect 处理完成后，将 iOS 与 macOS build 50 加入内部 TestFlight 群组并在真机验证登录、课表、空教室、日历导入、通知、深浅色和前后台切换。
