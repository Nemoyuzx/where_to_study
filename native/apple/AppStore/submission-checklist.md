...
- 当前预发布候选 iOS `0.2.8 (76)` 与 macOS `0.2.8 (75)` 已完成本地 Xcode 正式签名归档和分平台上传；两端均收到 `Upload succeeded`，iOS build 76 的本地 Apple Distribution 导出验证与脚本流程成功。
- build 75 包含独立一级“查询”导航；build 76 进一步纳入班车复核记录可空字段兼容，并通过生产 API 的 Store 与真实 App UI 测试。
...
- 分别在真机和实际 Mac 上继续验证 iOS build 76 / macOS build 75 的登录、课表、空教室、综合查询、日历导入、通知、深浅色和前后台切换。
