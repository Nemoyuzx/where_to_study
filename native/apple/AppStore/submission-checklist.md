...
- macOS 与 iOS `0.2.3 (64)` 已完成本地 Xcode 严格编译、逻辑/UI 回归和正式签名上传；两端均在 2026-08-24 收到 Xcode `Upload succeeded` 和 `EXPORT SUCCEEDED`，未再打开 App Store Connect 检查处理状态。
...
- iOS 与 macOS build 64 在 TestFlight 客户端可见后，分别在真机和实际 Mac 上验证登录、课表、空教室、日历导入、通知、深浅色和前后台切换。
