...
- macOS `0.1.6 (41)` 与 iOS `0.1.6 (43)` 的对应预检、Release 编译、Apple 逻辑测试及设备专项 UI 验证均已通过；两个主应用归档均已核对为 `com.nemoyu.wheretostudy.native.macos`，应用图标统一为 Windows/Tauri 当前使用的绿色日历课桌图标。
- iOS `0.1.6 (43)` 与 macOS `0.1.6 (41)` 均已收到 App Store Connect 的 `Upload succeeded`，上传完成。TestFlight 和正式提交分别选择 iOS build 43 与 macOS build 41。
...
- App Store Connect 处理完成后，将 iOS build 43 与 macOS build 41 加入内部 TestFlight 群组并在真机验证登录、课表、空教室、日历导入、通知、深浅色和前后台切换。
