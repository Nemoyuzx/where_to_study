...
- `preflight all`、macOS/iOS `APP_STORE_BUILD` Release 编译、Apple 逻辑测试、iPhone/iPad 专项 UI 验证、Universal macOS `0.1.4 (29)` 安装及正式签名归档均已通过；两个主应用归档均核对为 `com.nemoyu.wheretostudy.native.macos`，应用图标统一为 Windows/Tauri 当前使用的绿色日历课桌图标。
- iOS 与 macOS `0.1.4 (29)` 均已收到 App Store Connect 的 `Upload succeeded`；当前等待 Apple 处理。TestFlight 和正式提交只选择 build 29。
- Build 25 的旧审核截图清单保留作历史校验，不再用于提交；build 29 应按 `screenshot-plan.md` 重新生成 iPhone、iPad 与 macOS 商店截图并更新 `screenshot-manifest.md`。
...
- App Store Connect 处理完成后，将 build 29 加入内部 TestFlight 群组并在真机验证登录、课表、空教室、日历导入、通知、深浅色和前后台切换。
- 重新生成并上传 build 29 的商店截图，填入本目录的中英文审核说明与简体中文元数据，最后再选择 build 29 提交审核。
