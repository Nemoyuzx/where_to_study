...
- 正式归档前需安装有效的 Apple Distribution、Mac Installer Distribution 证书及 iOS 主应用、iOS Widget、macOS 主应用、macOS Widget 四个 App Store 描述文件；证书私钥和团队标识不写入仓库。
- macOS `0.1.9 (49)` 的预检、Release 归档、Apple 逻辑测试及设备专项 UI 验证均已通过；iOS 当前源码 build 49 的逻辑与 UI 回归也已通过。两个主应用标识均已核对为 `com.nemoyu.wheretostudy.native.macos`，应用图标统一为 Windows/Tauri 当前使用的绿色日历课桌图标。
- iOS `0.1.9 (48)` 与 macOS `0.1.9 (49)` 均已收到 Xcode 的 `Upload succeeded`，上传完成；按发布流程未再打开浏览器复核。TestFlight 和正式提交应分别选择对应平台的最新构建。
...
- App Store Connect 处理完成后，将 iOS build 48 与 macOS build 49 加入内部 TestFlight 群组并在真机验证登录、课表、空教室、日历导入、通知、深浅色和前后台切换。
