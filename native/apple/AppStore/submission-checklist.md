...
- macOS 与 iOS `0.2.6 (70)` 已完成本地 Xcode 严格编译、逻辑/UI 回归、正式签名归档和上传；两端均收到 `Upload succeeded` 与 `EXPORT SUCCEEDED`。
...
- iOS 与 macOS build 70 在 TestFlight 客户端可见后，分别在真机和实际 Mac 上验证登录、课表、空教室、日历导入、通知、深浅色和前后台切换。
