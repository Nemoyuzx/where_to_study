...
- macOS 与 iOS `0.2.5 (69)` 已完成本地 Xcode 严格编译、逻辑/UI 回归、正式签名归档和上传；两端均收到 `Upload succeeded` 与 `EXPORT SUCCEEDED`。
- 后续 TestFlight 发布以 Xcode 的 `Upload succeeded` 和 `EXPORT SUCCEEDED` 为完成标准，不再额外打开 App Store Connect 检查 processing 或测试组状态。`0.2.5` 的 iOS 与 macOS 均使用 Build 69。
...
- iOS 与 macOS build 65 在 TestFlight 客户端可见后，分别在真机和实际 Mac 上验证登录、课表、空教室、日历导入、通知、深浅色和前后台切换。
