...
- `0.2.2 (51)` 已完成本地 Xcode 的 macOS/iOS 编译、逻辑测试和 iPhone UI 回归；2026-08-23 两个平台的上传任务均收到 Xcode `EXPORT SUCCEEDED`，现已进入 App Store Connect / TestFlight processing。
...
- App Store Connect 处理完成后，将 iOS 与 macOS build 51 加入内部 TestFlight 群组并在真机验证登录、课表、空教室、日历导入、通知、深浅色和前后台切换。
