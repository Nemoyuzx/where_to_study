...
本地正式构建会校验非 ad-hoc 签名、版本/构建号、macOS App Sandbox、网络/日历 entitlement、双平台主应用与 Widget App Group、隐私清单及三份许可证文件。iOS Automatic 的中间 archive 使用 Development 签名；脚本会在上传前先生成同配置的本地 App Store export，确认主应用与 Widget 均已改为 Apple Distribution 签名且不含 `get-task-allow`。当前本机已验证路径是：iOS 设置 `APPLE_IOS_SIGNING_STYLE=Automatic` 使用 Xcode 管理的描述文件；iOS 成功后再以默认 Manual 模式上传 macOS，由脚本使用命名 App Store profile 并按团队查找 Installer Distribution 身份。不要手工重复执行 `archive → export → upload`；`upload` 已包含重新归档、iOS 本地导出验证和上传。iOS `0.2.8 (77)` 与 macOS `0.2.8 (75)` 已分别收到 `Upload succeeded`；按约定未继续检查 App Store Connect processing。GitHub Pre-release 同步提供未公证的原生 macOS `0.2.8 (75)` Universal DMG 预览包，不提供 iOS 制品。DMG 内应用文件名为 `WhereToStudyMac.app`，可覆盖同名 TestFlight 安装而不会创建第二个同 Bundle ID 副本。完整命令、常见签名误区、账户侧配置、元数据和审核确认见 [`AppStore/submission-checklist.md`](./AppStore/submission-checklist.md)。
