...
macOS 安装包内嵌真正的 WidgetKit extension，可在通知中心或桌面的小组件图库中添加“今日课程”。主应用在读取、刷新或清除个人课表时写入原子化的小组件快照并刷新 timeline；主应用与扩展使用一致的 App Group entitlement，同时为无 Developer ID 的本地 ad-hoc 包保留应用支持目录兼容副本。
