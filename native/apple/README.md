...
iOS 与 macOS 安装包都内嵌真正的 WidgetKit extension，可在系统小组件图库中添加“今日课程”。主应用在读取、刷新或清除个人课表时写入原子化的小组件快照并刷新 timeline；主应用与扩展使用一致的 App Group entitlement。当天没有课程或尚未获取课表时，小组件统一显示“今日无课”。正式 App Store 构建只使用已签名的 App Group 容器，本地 ad-hoc 预览包另保留应用支持目录兼容副本。
...
本地正式构建默认使用手动分发签名和已安装的四个 App Store 描述文件，校验非 ad-hoc 签名、版本/构建号、`get-task-allow`、macOS App Sandbox、网络/日历 entitlement、双平台主应用与 Widget App Group、隐私清单及三份许可证文件。macOS 导出会自动按团队查找 Installer Distribution 身份，也可通过 `APPLE_INSTALLER_SIGNING_CERTIFICATE` 覆盖；CI 可另外传入 `APPLE_AUTH_KEY_PATH`、`APPLE_AUTH_KEY_ID`、`APPLE_AUTH_KEY_ISSUER_ID`。iOS 与 macOS 当前发布构建均为 `0.1.8 (46)`；TestFlight 与正式提交选择对应平台的最新构建，Apple 制品不再作为 GitHub Release 附件。账户侧配置、商店元数据、隐私问卷草案、截图方案、App Review 说明和 GPL 发布确认见 [`AppStore/`](./AppStore/)。
