...
macOS 安装包内嵌真正的 WidgetKit extension，可在通知中心或桌面的小组件图库中添加“今日课程”。主应用在读取、刷新或清除个人课表时写入原子化的小组件快照并刷新 timeline；主应用与扩展使用一致的 App Group entitlement。正式 App Store 构建只使用已签名的 App Group 容器，本地 ad-hoc 预览包另保留应用支持目录兼容副本。
...
iOS 与 macOS 主应用共用 `com.nemoyu.wheretostudy.native.macos`，以便在 App Store Connect 中作为一个通用购买记录发布。macOS Widget 使用独立扩展标识符 `com.nemoyu.wheretostudy.native.macos.widget`。
