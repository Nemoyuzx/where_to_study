...
教学日历和设置均可进入“班车与重要事件查询”。班车使用固定 HTTPS API，只展示当天生效且通过严格校验的双向班次、下一班与发车地点；重要事件复用教学日历的公开 Contest DDL 和校内通知缓存，支持全文/真实分类筛选、显示已结束、DDL 升序和本地收藏，不包含课程作业或自定义日程。学术会议与期刊专题可通过独立会议开关进入教学日历；查询页首次出现并行预热数据，顶部滑块只切换本地视图。
...
本地正式构建会校验非 ad-hoc 签名、版本/构建号、macOS App Sandbox、网络/日历 entitlement、双平台主应用与 Widget App Group、隐私清单及三份许可证文件。iOS Automatic 的中间 archive 使用 Development 签名；脚本会在上传前先生成同配置的本地 App Store export，确认主应用与 Widget 均已改为 Apple Distribution 签名且不含 `get-task-allow`。当前本机已验证路径是：iOS 设置 `APPLE_IOS_SIGNING_STYLE=Automatic` 使用 Xcode 管理的描述文件；iOS 成功后再以默认 Manual 模式上传 macOS，由脚本使用命名 App Store profile 并按团队查找 Installer Distribution 身份。不要手工重复执行 `archive → export → upload`；`upload` 已包含重新归档、iOS 本地导出验证和上传。iOS `0.2.7 (73)` 与 macOS `0.2.7 (73)` 已分别收到 `Upload succeeded` / `EXPORT SUCCEEDED`；按约定未继续检查 App Store Connect processing。GitHub Release 同步提供未公证的原生 macOS `0.2.7 (73)` Universal DMG 预览包，不提供 iOS 制品。DMG 内应用文件名为 `WhereToStudyMac.app`，可覆盖同名 TestFlight 安装而不会创建第二个同 Bundle ID 副本。完整命令、常见签名误区、账户侧配置、元数据和审核确认见 [`AppStore/submission-checklist.md`](./AppStore/submission-checklist.md)。
