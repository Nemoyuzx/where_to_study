...
本地正式构建会校验非 ad-hoc 签名、版本/构建号、`get-task-allow`、macOS App Sandbox、网络/日历 entitlement、双平台主应用与 Widget App Group、隐私清单及三份许可证文件。当前本机已验证路径是：iOS 设置 `APPLE_IOS_SIGNING_STYLE=Automatic` 使用 Xcode 管理的描述文件；iOS 成功后再以默认 Manual 模式上传 macOS，由脚本使用命名 App Store profile 并按团队查找 Installer Distribution 身份。不要先跑 `archive → export` 再跑 `upload`，因为 `upload` 会重新归档。iOS `0.2.6 (72)` 与 macOS `0.2.6 (72)` 已分别收到 `Upload succeeded` / `EXPORT SUCCEEDED`；按约定未继续检查 App Store Connect processing。GitHub Release 同步提供未公证的原生 macOS `0.2.6 (72)` Universal DMG 预览包，不提供 iOS 制品。DMG 内应用文件名为 `WhereToStudyMac.app`，可覆盖同名 TestFlight 安装而不会创建第二个同 Bundle ID 副本。完整命令、常见签名误区、账户侧配置、元数据和审核确认见 [`AppStore/submission-checklist.md`](./AppStore/submission-checklist.md)。
