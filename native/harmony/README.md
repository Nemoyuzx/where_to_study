...
```bash
./scripts/native-harmony-build.sh      # assembleHap/assembleApp + 132 个契约单元测试
...
> 的 assembleHap/assembleApp 编译与 132 个契约单元测试；Pura 90 仿真器上的手机 UI
> 冒烟测试 21/21、月视图双向翻页与折叠设备测试 1/1、隔离 alias 的 ASSET 新增/读取/更新/删除测试 1/1 均通过。已上传的 0.2.8 (1002017) Release APP/HAP 另已通过 SHA-256、
> 独立 HAP 签名、APP ZIP 结构、版本和三项固定 HTTPS API 校验。DevEco“上传产品”已将
> 0.2.8 (1002017) 上传 AppGallery Connect 用于测试和发布，云测试结果为“通过”。HarmonyOS 安装包仅通过 AppGallery Connect 分发，不上传 GitHub Release。
