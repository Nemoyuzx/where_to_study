...
课前提醒与每日摘要分别开关、默认关闭。提前量默认 `[10]`，可设为 1–5 次不重复的整数分钟（每次 1–1440 分钟）；两类请求按时间合并占用最多 30 条系统提醒代理容量。仅基于本地有效课表规划，支持明确时间的考试，跳过未知时间，前台补排且不补发过期提醒。单条发布失败有限重试、保留其他成功与后续请求，不因临时错误关闭用户开关。详细行为与限制见[课前提醒说明](../../docs/pre-class-reminders.md)。
