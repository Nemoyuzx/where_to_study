...
- 应用版本：`0.1.4`
- 当前开发构建号：Apple `CURRENT_PROJECT_VERSION=28`；Android `versionCode=17`
- 教务数据源：只使用现有移动教务 SJD HTTPS 接口，没有切换或静默回退到其他数据源
- 本地安装：只保留 SwiftUI Universal 应用 `/Applications/Where To Study.app`
- 发布边界：`v0.1.4` 使用稳定版本号；Apple Developer 标识符、App Group、分发证书、描述文件和双平台 App Store Connect 记录已配置，正式签名的 iOS/macOS build 28 已上传并等待处理；公开 GitHub Release 的 macOS/iOS 制品仍分别为 ad-hoc 与无签名构建；项目按 GPL-3.0-only 开源
...
| macOS 安装检查 | SwiftUI Universal `0.1.4 (28)` 已覆盖安装到唯一位置 `/Applications/Where To Study.app`；x86_64/arm64、ad-hoc 签名、WidgetKit 扩展、版本与统一应用图标均复核通过 |
| App Store Connect | iOS 与 macOS `0.1.4 (28)` 均完成 Apple Distribution 签名、描述文件/entitlement 校验并收到 `Upload succeeded`；当前等待 App Store Connect 处理，不等同于已通过审核或正式发布 |
...
1. 等待 iOS/macOS build 28 处理完成，将 build 28 加入内部 TestFlight 群组并完成真机安装与核心流程验证。
