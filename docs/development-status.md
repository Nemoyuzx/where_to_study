...
- 平板、折叠屏和 PC 宽屏日历、设置布局与现有原生端保持功能层级一致；构建及 79 项 ArkTS 单元测试通过。
...
| Rust | 共享核心、Tauri、CLI、TUI 的既有门禁保持通过；本轮 Tauri `fmt`、`clippy -D warnings` 与 115/115 自动测试通过，另 1 项需本机安全存储和北邮在线服务的真实同步测试按设计忽略，并已单独验证通过 |
| Rust 依赖审计 | `cargo audit 0.22.2`：0 个漏洞；17 个允许警告来自 Tauri 的 Linux GTK3/旧 proc-macro/unic 传递依赖 |
| macOS SwiftUI | 严格 Swift 6 并发；136/136 XCTest 通过，并实机缩放检查教学日历会铺满可用区域 |
| iOS SwiftUI | 严格 Swift 6 并发；137/137 逻辑测试通过；UI 测试套件共 12 项、iPad 专项跳过 1 项、其余 11 项通过 |
| Android Debug | 152/152 JVM 测试、Lint、Debug APK 与 AndroidTest APK 构建通过 |
...
| HarmonyOS | HAP 构建、79/79 ArkTS 单元测试和宽屏布局静态契约通过 |
...
- macOS：136/136 通过
- iOS：137/137 项逻辑测试通过；UI 测试套件共 12 项、iPad 专项跳过 1 项、其余 11 项通过
