...
- 应用版本：`0.1.4`
- 当前开发构建号：Apple `CURRENT_PROJECT_VERSION=34`；Android `versionCode=18`
...
- 发布边界：`v0.1.4` 使用稳定版本号；Apple Developer 标识符、App Group、分发证书、描述文件和双平台 App Store Connect 记录已配置，正式签名的 iOS build 35 与 macOS build 34 已上传并等待处理；公开 GitHub Release 的 macOS/iOS 制品仍分别为 ad-hoc 与无签名构建；项目按 GPL-3.0-only 开源
...
| iOS SwiftUI | 严格 Swift 6 并发、警告视为错误；92 项单元测试通过，iPhone UI 套件与 11/13 英寸 iPad 横竖屏专项回归通过 |
| Android Debug | 114/114 JVM 测试、Debug APK 与 AndroidTest APK 构建通过 |
...
| macOS 安装检查 | SwiftUI Universal `0.1.4 (33)` 已覆盖安装到 `/Applications/Where To Study.app`；x86_64/arm64、ad-hoc 签名、WidgetKit 扩展、版本与统一应用图标均复核通过；未检测到其他安装副本 |
| App Store Connect | iOS `0.1.4 (35)` 与 macOS `0.1.4 (34)` 均完成 Apple Distribution 签名、描述文件/entitlement 校验并收到 `Upload succeeded`；当前等待 App Store Connect 处理，不等同于已通过审核或正式发布 |
...
Apple 测试结果（2026-08-16 使用 `xcresulttool` 复核）：

- macOS：92/92 通过
- iOS：92 项单元测试通过；iPhone 主 UI 套件、11/13 英寸 iPad 横竖屏验证及新增移动日历交互回归分别通过
...
1. 等待 iOS build 35 与 macOS build 34 处理完成，将对应构建加入内部 TestFlight 群组并完成真机安装与核心流程验证。
