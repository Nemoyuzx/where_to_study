...
- 应用版本：`0.1.6`
- 当前开发构建号：Apple `CURRENT_PROJECT_VERSION=42`；Android `versionCode=23`
...
- 发布边界：`v0.1.6` 使用稳定版本号；Apple Developer 标识符、App Group、分发证书、描述文件和双平台 App Store Connect 记录已配置，正式签名的 iOS 使用 build 42、macOS 使用 build 41；公开 GitHub Release 的 macOS/iOS 制品仍分别为 ad-hoc 与无签名构建；项目按 GPL-3.0-only 开源
...
| iOS SwiftUI | 严格 Swift 6 并发、警告视为错误；108 项单元测试通过，iPhone 竖屏、横屏与 iPad 常规宽度侧栏专项 UI 回归 3/3 通过 |
...
| App Store Connect | iOS `0.1.6 (42)` 与 macOS `0.1.6 (41)` 均已使用 Apple Distribution 签名并校验描述文件/entitlement；两个平台的上传任务均收到 `Upload succeeded` |
...
- macOS：106/106 通过
- iOS：108 项单元测试通过；iPhone 竖屏、横屏与 iPad 常规宽度侧栏专项 UI 回归 3/3 通过
...
1. 等待 iOS build 42 与 macOS build 41 处理完成，将对应构建加入内部 TestFlight 群组并完成真机安装与核心流程验证。
