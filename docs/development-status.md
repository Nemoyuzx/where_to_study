...
- 托盘提供今日/明日课程；关闭主窗口后保持运行；启动和每天 07:00 获取当天空教室；课程摘要默认关闭，时间可自定义（北京时间，默认 07:30），保存后唤醒调度器应用新时间。
...
- 可选本地课程摘要支持自定义北京时间（默认 07:30），会在系统上限内安排最多 63 个未来有课日；关闭提醒、撤销权限、切换账号或清除数据会立即撤销旧通知。
...
- iOS/macOS 今日课程 Widget 提供课程数量和教师信息偏好，并在设置页提供与实际尺寸一致的预览；优先显示今日，实际高度有剩余时追加“明日课程”，当天无课时仍显示“今日无课”。
...
- 可选课程摘要使用持久化 `JobScheduler`，支持自定义北京时间（默认 07:30），只在所选时间起的有限窗口内投递且不跨午夜；关闭、改时、撤销权限、切换账号和清除数据均使旧任务失效。
...
- “今日课程”服务卡片补齐课程数量、地点和教师展示偏好，并统一无课状态；按真实尺寸优先今日，剩余空间显示带明日分组的课程。每日提醒支持自定义北京时间，规划与系统提交使用同一绝对时刻。
...
| HarmonyOS | `0.2.8 (1002021)` 的 139/139 ArkTS 单元测试、签名 HAP/APP、打包版本、发布签名和三项 API 校验通过；周→月日期条使用动画目标模式同步切换，月视图最高档保持完整六行网格并通过视口裁剪/平移显示选中周；DevEco 已上传 AGC 且云测试为“通过”。本机模拟器镜像当前不可用，本次交互视觉验证由逻辑几何回归与 DevEco 云测试覆盖 |
