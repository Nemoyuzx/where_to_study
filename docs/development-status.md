...
- 应用版本：`0.2.2`
- 当前开发构建号：Apple `CURRENT_PROJECT_VERSION=56`；Android `versionCode=32`
...
- 发布边界：`v0.2.2` 使用稳定版本号；Apple Developer 标识符、App Group、分发证书和双平台 App Store Connect 记录已配置，iOS build 56 与 macOS build 52 只通过 TestFlight 分发；GitHub Release 不上传任何 iOS、macOS 或 `.sha256` 文件；项目按 GPL-3.0-only 开源
...
- Android 月视图日期详情已接入云课堂原生 CAS 登录与实时作业同步、黄历宜忌和统一活动 DDL 卡片；天气、黄历、学科竞赛、校内竞赛通知、夏令营和黑客松六个开关与来源声明和 Apple 端一致。手机日期详情到达最高档后可继续滚动到底，滚动视口使用圆角边缘遮罩约束卡片层级；日期选择使用单套月格的行高、详情显隐和轻量方向位移连续过渡，不叠加两层日期；折叠屏收起侧栏时图标在选中框内保持纵向居中。
...
- 平板、折叠屏和 PC 宽屏日历、设置布局与现有原生端保持功能层级一致；构建及 80 项 ArkTS 单元测试通过。

## 最终本地验证

| 范围 | 结果 |
| --- | --- |
| React | 56/56 业务规则、主题契约、Windows/Linux 无伪小组件、Android 折叠侧栏居中、图标安全区、Linux 发布契约与全端版本一致性测试、`npm run build`、许可证新鲜度检查通过 |
...
| iOS SwiftUI | 本机 Xcode 严格 Swift 6 并发；141/141 逻辑测试通过；iPhone UI 套件共 13 项，其中 2 项 iPad 专项按设计跳过、其余 11 项通过；月视图事件条点击和动画中快速改选日期回归通过 |
| Android Debug | 153/153 JVM 测试、Lint、Debug APK 与 AndroidTest APK 构建通过 |
| Android Release | `0.2.2 (32)` 的 Release JVM 测试、`lintRelease`、固定证书签名 APK/AAB、证书指纹、ZIP 对齐、许可证与包内版本校验通过，并已替换 GitHub Release 中对应的两个 Android 资产 |
...
| App Store Connect | iOS `0.2.2 (56)` 与 macOS `0.2.2 (52)` 均完成正式签名上传；iOS Build 56 收到 Xcode `Upload succeeded` 与 `EXPORT SUCCEEDED`，按当前发布约定未再打开 App Store Connect 检查后续状态 |
...
`v0.2.2` 的 GitHub Release 提供 Windows x64 NSIS、Linux arm64/x86_64 Debian/AppImage/CLI/TUI，以及固定 release key 签名的 Android `0.2.2 (32)` APK/AAB。iOS `0.2.2 (56)` 与 macOS `0.2.2 (52)` 已上传 TestFlight，不进入 GitHub Release；脚本或 CI 生成的 `.sha256` 只供内部校验，同样不上传。
