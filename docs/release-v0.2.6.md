...
- GitHub Release 恢复原生 macOS `0.2.6 (70)` Universal DMG，同时继续提供 Windows、Linux、CLI/TUI、Android 和 HarmonyOS 制品。DMG 包含 `arm64 + x86_64`、标准 Applications 快捷方式并通过签名和镜像校验，但未使用 Developer ID 公证；优先推荐 build 71 TestFlight 版本。
...
- GitHub Release retains the native macOS `0.2.6 (70)` universal DMG alongside Windows, Linux, CLI/TUI, Android, and HarmonyOS artifacts. The DMG contains `arm64 + x86_64`, an Applications shortcut, and passed signature/image validation, but is not Developer-ID notarized; build 71 TestFlight remains the recommended macOS channel.
