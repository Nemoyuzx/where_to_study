...
- Apple build 71 继续将月/年快照改为有界多键缓存，预计算日期、无障碍文案与 DDL 边框，并把 macOS 年视图从 504 个日期槽收敛为 365/366 个真实日期；同一 18 次月/年快速切换采样中未再捕获 microhang。
- Tauri 完成宽/窄屏、深/浅色、中/英文的日/周/月/年、设置、收藏和空教室视觉矩阵，并修复窄屏操作栏与收藏空页布局。

### 版本与分发

- Android：`0.2.6 (42)`，固定维护者证书签名的 Universal APK/AAB。
- iOS 与原生 macOS：`0.2.6 (71)`，本地 Xcode 归档并上传 TestFlight；两端均收到 `Upload succeeded` 与 `EXPORT SUCCEEDED`，按约定未继续检查 App Store Connect processing。
...
- GitHub Release 的原生 macOS Universal DMG 已刷新为 `0.2.6 (71)`，其它 Windows、Linux、CLI/TUI、Android 和 HarmonyOS 制品保持不变。DMG 包含 `arm64 + x86_64`、标准 Applications 快捷方式并通过签名和镜像校验，内部应用名与 TestFlight 安装一致；该预览包未使用 Developer ID 公证。

### 验证

- Node 契约与主题测试：114/114。
- Tauri Rust：146 通过，1 个需要真实账号和在线服务的测试按设计忽略。
- Apple：macOS 210 项通过；iOS 236 项通过、4 项按设备条件跳过且 0 失败；iPhone 核心 UI、iPad Pro 13 英寸横屏与 macOS 视觉/滚动回归通过。
...
- Apple build 71 further uses a bounded multi-key snapshot cache, precomputes date/accessibility/deadline-border projections, and reduces the macOS year view from 504 calendar slots to 365/366 real dates. No microhang was captured in the repeated 18-switch month/year profiling pass.
...
- Native iOS and macOS: `0.2.6 (71)`, archived and uploaded with local Xcode. Both uploads returned `Upload succeeded` and `EXPORT SUCCEEDED`; App Store Connect processing was intentionally not inspected afterward.
...
- The GitHub Release native macOS universal DMG was refreshed to `0.2.6 (71)` while the Windows, Linux, CLI/TUI, Android, and HarmonyOS assets remain unchanged. The DMG contains `arm64 + x86_64`, an Applications shortcut, uses the same app filename as TestFlight for replacement installs, and passed signature/image validation; it is not Developer-ID notarized.

### Validation

- Node contracts/theme: 114/114.
- Tauri Rust: 146 passed, 1 live-account test intentionally ignored.
- Apple: 210 macOS tests passed; iOS recorded 236 passes, 4 device-conditional skips, and 0 failures, including core iPhone UI, 13-inch iPad landscape, and macOS visual/scroll regressions.
