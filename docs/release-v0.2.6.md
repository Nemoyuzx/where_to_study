...
- HarmonyOS build 1002011 将桌面快捷键焦点限制在宽屏侧栏，恢复手机账号/密码输入框的触摸焦点与系统输入法；键盘弹出后设置页使用 RESIZE 避让并保持可滚动。
...
- HarmonyOS：`0.2.6 (1002011)`，Release 签名 HAP/App 构建、签名摘要验证与 113 项单元测试通过；DevEco 已更新上传 AppGallery Connect 的 `0.2.6` 测试版本且云测试通过。GitHub 同时保留已验证签名资产；[打开 HarmonyOS 测试邀请（链接已含邀请码）](https://appgallery.huawei.com/link/invite-test-wap?taskId=dfc32d0293987b9d09911717759ac063&invitationCode=A0IsJpKIcn3)，邀请码为 `A0IsJpKIcn3`。
- GitHub Release 的原生 macOS Universal DMG 保持 `0.2.6 (72)`，HarmonyOS APP/HAP 刷新为 `0.2.6 (1002011)`；Windows、Linux、CLI/TUI 和 Android 资产因产品源码未变化而保持不变。DMG 包含 `arm64 + x86_64`、标准 Applications 快捷方式并通过签名和镜像校验；该预览包未使用 Developer ID 公证。`v0.2.6` 标签不强制移动，刷新资产来自标签后的 `main` 修复。
...
- HarmonyOS：113/113 ArkTS 单元测试、Pura 90 手机 16/16 UI 冒烟、两项输入框设备测试、MateBook Pro 宽屏 5/5 导航回归、Release APP/HAP 构建与签名摘要验证通过。
- CLI/TUI：各 14 项测试通过。
...
- HarmonyOS: `0.2.6 (1002011)`, with Release-signed HAP/App builds, signature verification, 113 unit tests, phone IME regressions, and wide-layout navigation checks passed. DevEco refreshed the `0.2.6` AppGallery Connect test build and its cloud test passed. [Open the HarmonyOS test invitation (code included)](https://appgallery.huawei.com/link/invite-test-wap?taskId=dfc32d0293987b9d09911717759ac063&invitationCode=A0IsJpKIcn3); the invitation code is `A0IsJpKIcn3`.
