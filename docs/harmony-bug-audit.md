...
| BUG-040 | 卡片按 formDimension 差异化限行（2*2 两行，其余三行） | 🔧 |
| BUG-051 | withTimeout 超时后记录底层操作迟到结果（hilog 对账） | 🔧 |
| BUG-053/054 | 卡片颜色改用系统资源色（ohos_id_color_*），深浅色自适应 | 🔧 |
...
| BUG-053/054 | 卡片与状态条颜色已改系统资源色（ohos_id_color_*），随深浅色切换 |
