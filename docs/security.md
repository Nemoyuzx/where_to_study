...
- 只向产品所需的北邮教务、节假日与 UAPI 天气/黄历数据源发送请求。
...
- 天气与黄历数据：只向 `https://uapis.cn` 发送固定 HTTPS 请求；天气仅提交西土城对应海淀区或沙河对应昌平区的公开行政区划代码，黄历仅提交所选日期换算的时间戳与 `Asia/Shanghai` 时区。重定向必须保持 HTTPS 和同一主机，响应大小受限，不得携带教务凭据、课表或空教室数据。
