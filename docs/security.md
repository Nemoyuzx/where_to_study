...
- 公开 DDL：主数据只从 `https://nemoyuzx.github.io/contest-ddl/data/competitions.json` 获取并在本地按日期和类型筛选。备用活动与校内竞赛通知分别固定为 `https://where-to-study.cn/api/contest-events` 和 `https://where-to-study.cn/api/contest-notices`；只能发送不含任何凭据、Cookie、token、课表、教室或作业数据的 HTTPS `GET`，必须拒绝重定向并限制响应体。校内通知的详情链接只接受 HTTPS；任何平台都不得为此开放明文传输例外。
- 班车：只允许从固定的 `https://where-to-study.cn/api/shuttle-bus` 获取 Schema 1.0 JSON，拒绝 `www` 跳转和任何其他重定向，限制响应体并验证后勤部原文主机、运行日期、星期、`HH:mm`、车型与车辆数量。客户端只展示 `parse_status=parsed` 的结构化班次；最新通知未安全解析时可明确标注并回退上一份完整表，但不得把低置信度结果当作当前班次。请求不得携带凭据、Cookie、token、课表、校区选择或位置数据。
