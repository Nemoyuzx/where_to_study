...
- 公开 DDL：主数据只从 `https://nemoyuzx.github.io/contest-ddl/data/competitions.json` 获取并在本地按日期和类型筛选。备用地址仅允许用户指定的 `http://101.201.29.29/api/contest-events`；明文例外必须固定到该主机与默认 80 端口，只能发送不含任何凭据、Cookie、token、课表、教室或作业数据的 `GET`，必须拒绝重定向并限制响应体。不得因此对教务请求或其他主机开放全局明文传输。
