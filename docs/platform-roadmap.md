...
- 原生分发限制：每日课程摘要已迁移；Android 使用固定维护者密钥签名发布；Apple 正式分发签名已上传 iOS `0.1.6 (42)` 与 macOS `0.1.6 (41)`，公开 GitHub Release 的 macOS/iOS 资产仍分别为 ad-hoc 与无签名构建；Windows/Linux 可信签名链路尚未闭环。
