...
- macOS/iOS 原生：共享 SwiftUI 已接入移动教务个人课表、Keychain、本地缓存、法定节假日、日/周/月/年视图、当前时间线、每日课程摘要、默认折叠的两校区天气卡片、黄历宜忌、公开活动 DDL、云课堂原生登录与作业同步及当天两校区空教室联动查询；通知取消、权限撤销、账号切换和 63 条系统上限已有自动化测试，主导航已通过 iOS UI 测试。
- Android 原生：Kotlin + Android Views 已接入移动教务个人课表、Android Keystore、`AtomicFile` 缓存、法定节假日、日/周/月/年视图、当前时间线、每日课程摘要、默认折叠的两校区天气卡片、黄历宜忌、公开活动 DDL、云课堂原生登录与作业同步及当天两校区空教室联动查询；通知持久撤销、有效送达窗口、后台中断和主导航已有自动化测试。
- 鸿蒙原生：ArkTS + ArkUI 已参考 SwiftUI 实现移植空教室、教学日历、设置三个一级页面，以及 ASSET 安全存储凭据、移动教务课表/空教室/节假日/天气客户端、默认折叠的天气卡片、黄历宜忌与公开活动 DDL、云课堂原生登录与作业同步、每日课程摘要（reminderAgent，上限 30 条）、Calendar Kit 系统日历导入与“今日课程”服务卡片；工程在 DevEco Studio 6.1.1 / API 24 上构建通过，79 个单元测试与手机、折叠屏、平板、PC 宽屏 UI 冒烟断言全部通过。折叠屏与电脑端适配已完成：Mate X7 折叠屏展开态（侧栏+宽布局）、MateBook Pro 2in1（侧栏导航、双列空教室、宽屏日/周/月/年日历）、600x900 悬浮窄窗（标签栏+紧凑单列回退）均已实测验证；半折叠降级规则覆盖单测。待补：发布签名与 AGC 上架（需维护者华为开发者账号）。
