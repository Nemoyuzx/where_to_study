...
- 原生分发限制：每日课程摘要已迁移；Android `0.2.6 (42)` 使用固定维护者密钥签名发布；Apple iOS 与 macOS `0.2.6 (70)` 均由本地 Xcode 上传 TestFlight；GitHub Release 恢复未公证的原生 macOS Universal DMG 预览包，Windows/Linux 可信签名链路尚未闭环。
