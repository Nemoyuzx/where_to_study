...
- 原生分发限制：每日课程摘要已迁移；Android 使用固定维护者密钥签名发布；Apple 已完成正式签名归档并上传 iOS/macOS `0.1.4 (30)`，等待 App Store Connect 处理和正式提交，但公开 GitHub Release 的 macOS/iOS 资产仍分别为 ad-hoc 与无签名构建；Windows 可信签名链路尚未闭环。
