...
- 鸿蒙原生：ArkTS + ArkUI 已参考 SwiftUI 移植三页主界面、ASSET 凭据、移动教务、天气/黄历、公开与校内竞赛、云课堂作业、课程摘要、Calendar Kit 和“今日课程”服务卡片；手机、折叠屏、平板与 PC 已同步双层 DDL 边框、完整本地收藏、收藏管理、自定义 HTTPS 日程源、教学周、键盘操作、范围缓存及中英文界面。DevEco Studio 6.1.1 / API 24 的 HAP 与 94 项单元测试通过；待补正式发布签名与 AGC 上架。
- 阶段 2 状态：同一脱敏接口响应已在 Rust、Swift、Kotlin 和 ArkTS 生成一致的教学楼、三位教室号、双门教室号、座位数与可用节次；启动时仅在当天缓存缺失且已有凭据时刷新，不进行高频轮询。
- 原生分发限制：每日课程摘要已迁移；Android `0.2.5 (38)` 使用固定维护者密钥签名发布；Apple iOS 与 macOS `0.2.5 (66)` 均由本地 Xcode 上传 TestFlight，且不放入 GitHub Release；Windows/Linux 可信签名链路尚未闭环。
