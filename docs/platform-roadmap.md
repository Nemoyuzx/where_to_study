...
- 鸿蒙原生（feature/harmonyos 分支）：ArkTS + ArkUI 已参考 SwiftUI 实现移植空教室、教学日历、设置三个一级页面，以及 ASSET 安全存储凭据、移动教务课表/空教室/节假日客户端、每日课程摘要（reminderAgent，上限 30 条）、Calendar Kit 系统日历导入与“今日课程”服务卡片；工程在 DevEco Studio 6.1.1 / API 24 上构建通过，41 个契约单元测试（复用 contracts/v1 fixtures）全部通过，并已在 Pura 90 模拟器（HarmonyOS 6.1.1）完成安装、启动与页面导航验证。尚未合入发布流程，待补：UI 冒烟自动化与发布签名。
