...
- Windows/Tauri：功能最完整，凭据已迁移到系统安全存储；月视图的星期栏、日期、周数、事件行、字号、间距和选中态已按原生 macOS 几何对齐，并使用双层 DDL 同心边框；React 单文件仍需拆分。
- Linux/Tauri：与 Windows 共享 React/Rust 功能，使用 Ubuntu 22.04 原生 CI 发布 x86_64 Debian 包与 AppImage。
- macOS/iOS 原生：共享 SwiftUI 已接入移动教务个人课表、Keychain、本地缓存、法定节假日、日/周/月/年视图、当前时间线、每日课程摘要、默认折叠的两校区天气卡片、黄历宜忌、公开活动 DDL 与北邮校内竞赛通知、云课堂原生登录与作业同步及当天两校区空教室联动查询；月/年日期格用最多两层同心边框显示不同 DDL 分类，今天和选中日期保持独立状态；通知取消、权限撤销、账号切换和 63 条系统上限已有自动化测试，主导航已通过 iOS UI 测试。
- Android 原生：Kotlin + Android Views 已接入移动教务个人课表、Android Keystore、`AtomicFile` 缓存、法定节假日、日/周/月/年视图、当前时间线、每日课程摘要、默认折叠的两校区天气卡片、黄历宜忌、公开活动 DDL 与北邮校内竞赛通知、云课堂原生登录与作业同步及当天两校区空教室联动查询；月/年日期格使用 `1.2dp` 外框与 `0.8dp` 内框表达前两类 DDL，今天和选中日期独立显示；通知持久撤销、有效送达窗口、后台中断和主导航已有自动化测试。
- 鸿蒙原生：ArkTS + ArkUI 已参考 SwiftUI 实现移植空教室、教学日历、设置三个一级页面，以及 ASSET 安全存储凭据、移动教务课表/空教室/节假日/天气客户端、默认折叠的天气卡片、黄历宜忌、公开活动 DDL 与北邮校内竞赛通知、云课堂原生登录与作业同步、每日课程摘要（reminderAgent，上限 30 条）、Calendar Kit 系统日历导入与“今日课程”服务卡片；工程在 DevEco Studio 6.1.1 / API 24 上构建通过，手机、折叠屏、平板、PC 宽屏 UI 冒烟断言全部通过。手机日/周/月/年已加入作业与校内竞赛日程、双层 DDL 同心边框、独立的今天/选中状态、折叠课程区、范围缓存及中英文界面。折叠屏与电脑端适配已完成：Mate X7 折叠屏展开态（侧栏+宽布局）、MateBook Pro 2in1（侧栏导航、双列空教室、宽屏日/周/月/年日历）、600x900 悬浮窄窗（标签栏+紧凑单列回退）均已实测验证；半折叠降级规则覆盖单测。待补：发布签名与 AGC 上架（需维护者华为开发者账号）。
- 阶段 2 状态：同一脱敏接口响应已在 Rust、Swift、Kotlin 和 ArkTS 生成一致的教学楼、三位教室号、双门教室号、座位数与可用节次；启动时仅在当天缓存缺失且已有凭据时刷新，不进行高频轮询。
- 原生分发限制：每日课程摘要已迁移；Android `0.2.4 (37)` 使用固定维护者密钥签名发布；Apple iOS 与 macOS `0.2.4 (65)` 均已由本地 Xcode 上传 TestFlight，且不放入 GitHub Release；Windows/Linux 可信签名链路尚未闭环。
