...
- 原生分发限制：每日课程摘要已迁移；Android `0.2.6 (42)` 使用固定维护者密钥签名发布；Apple iOS 与 macOS `0.2.6 (72)` 均由本地 Xcode 分平台上传 TestFlight；GitHub Release 同步提供未公证的原生 macOS `0.2.6 (72)` Universal DMG 与 HarmonyOS `0.2.6 (1002011)` 签名 APP/HAP，Windows/Linux 可信签名链路尚未闭环。
