...
- iOS 与 macOS `0.2.8 (75)` 均由本地 Xcode 完成归档、导出和上传，两个平台均收到 `Upload succeeded`。
...
- Apple：本地 Xcode 严格构建通过；macOS 229/229，iOS 共 260 项（256 通过、4 项仅 iPad 条件跳过、0 失败）。
...
- iOS and macOS `0.2.8 (75)` were archived, exported, and uploaded from local Xcode; both platforms returned `Upload succeeded`.
...
- Apple: strict local-Xcode builds passed; macOS 229/229 and iOS 260 total tests (256 passed, four iPad-only conditional skips, zero failures).
