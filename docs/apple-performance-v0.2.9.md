...
- A final iPhone rerun with an isolated result bundle passed **4/4**: primary navigation, important-event queries, favorites and the slowed month/year/day hierarchy-transition flow.
- The live → demo → live UI flow was additionally checked after upload with a weather-recovery assertion; **1/1 passed** without changing application code. Returning to Planner restarts its view-scoped weather task and populates the separate mode store.
