...
- **华为 Android 已提交**：2026-09-12 用户恢复 Edge 登录后，确认原 (48) 已由用户提交审核。撤回该待审版本后上传同一签名 APK **0.2.9 (49)**，软件包行和提交确认弹窗均显示 (49)；保存成功后确认提交，页面返回“您的应用已提交审核”并显示 **正在审核**。沿用原发布区域、审核通过立即上架、权限、隐私网址和既有审核材料；此次旧授权附件正常通过校验，未重复上传或更改承诺函。已上架 0.2.8 未动。
- **HarmonyOS 已上传并提交**：DevEco 隔离工程 `/Users/nemoyu/Desktop/code/where_to_study-android029-hotfix/native/harmony` 通过第一项“生成.app包并上传至AppGallery Connect进行测试和发布”完成 **0.2.9 (1002030)** 上传，原生结果页显示 **云测试结果：通过**；AGC 软件包行上传时间为 **2026-09-12 10:59:07 +0800**。随后撤回原 (1002029) 待审版本，选取 (1002030)，更新布局修复说明并保存、确认提交，页面显示 **正在预审**。沿用原包加密、发布地区及上架设置；未上传 GitHub，未重复 Apple/vivo 上传，未包含成绩／考试功能。

本次 DevEco 新工程默认 Build Mode 不可上传，需在“选择产品”中设为 `release` 并应用；这是 IDE 状态，不能仅凭先前 CLI 已构建 Release 就认为上传向导已切换。该下拉层触发原生自动化读取失效后，用户手动设置并应用，随后按授权正常重启 DevEco，确认仍为隔离工程再上传。没有强制结束进程、改签名或修改源代码来绕过检查。

DevEco 实际上传后产物已在本地保存并与输出原件 `cmp` 一致，APP／HAP 的 `pack.info` 均为 `0.2.9 (1002030)`、build `1`，Release 模式校验通过；`termGPA` 未出现在包中。副本位于 ignored `release-artifacts/layout-hotfix-029/harmony-deveco1002030/`：

| 产物 | Bytes | SHA-256 |
| --- | ---: | --- |
| DevEco APP (1002030) | 1,312,299 | `0a830219cc4bb8a55cec4aaa8105467f6aec4fbd5ae46ac15395d878360e3216` |
| DevEco HAP (1002030) | 1,967,088 | `c61af62259ed4cda47ca14b43eca6ac702b8087b1b547ec6374569f275b6ac55` |

上传／云测试通过和提交审核不等于已经上架；本轮不等待人工审核结果，也不重复上传已成功的构建。
...
商店状态分别见下面最终上传记录：Apple 两端 (92) 上传成功，vivo (48) 审核中，鸿蒙 (1002029) 已由预审进入 **正在等待审核**；华为 Android (48) 已上传保存，仍需确认重传原承诺函后提交审核。上传、预审、人工审核和正式上架是不同阶段。下次上传须采用未用过的递增构建号，不重复本次成功的 Apple/DevEco 上传。
...
- **HarmonyOS**：最终 **0.2.9 (1002029)** 通过 DevEco 第一项“测试和发布”上传；AGC 软件包行记录 **21:50:55 +0800**，DevEco 显示 **云测试结果：通过**。已绑定至版本 `2037251738360139840` 并再次提交，提交后显示预审中，GitHub 发布后的最终刷新显示 **正在等待审核**；此前 (1002028) 的预审已撤回。没有重复使用测试专用上传选项。
