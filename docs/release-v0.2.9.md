...
- **HarmonyOS 尚未上传**：本地 0.2.9 (1002030) Release APP／HAP 编译、198 项测试和 `pack.info` 检查通过。用户已完成隔离工程信任及网页登录；已确认 DevEco 工程路径为 `/Users/nemoyu/Desktop/code/where_to_study-android029-hotfix/native/harmony`。刷新 DevEco 用户会话后，上传列表已恢复准确的 Where To Study 应用，但 IDE 的默认 Build Mode 阻止上传。自动化在选择构建模式后重复返回空窗口，用户授权的正常重启可恢复读取，但选择菜单后问题再次发生；已请求用户仅手动设置 `release` 并点“应用”，未发起鸿蒙上传，也未撤回原 1002029 待审版本。继续时仍用“上传产品 → 测试和发布”，上传后记录实际生成的包，不能从含未发布功能的主工作区上传同号包。
