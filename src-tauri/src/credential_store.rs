...
pub fn load() -> ServiceResult<Option<Credentials>> {
    ...
    let payload = match generic_password(PasswordOptions::new_generic_password(
        SERVICE_NAME,
        ENTRY_NAME,
    )) {
        Ok(payload) => payload,
...
#[cfg(target_os = "android")]
pub fn load() -> ServiceResult<Option<Credentials>> {
    let payload = match android_entry()?.get_secret() {
        Ok(payload) => payload,
...
pub fn save(credentials: &Credentials) -> ServiceResult<()> {
    ...
    let mut payload = serde_json::to_vec(credentials)
        .map_err(|error| ServiceError::new(format!("无法序列化账户凭据：{error}")))?;
...
mod tests {
    ...
    fn credential_payload_round_trips_as_structured_json() {
        ...
        let payload = serde_json::to_string(&credentials).expect("serialize fixture credentials");
