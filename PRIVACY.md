...
- **Weather and almanac data:** The app requests today's and tomorrow's weather for the selected campus district and lunar-calendar information for the selected teaching-calendar date from UAPI over HTTPS. Weather requests contain only the public administrative division code for Haidian (Xitucheng) or Changping (Shahe). Almanac requests contain only the selected date as a timestamp and the `Asia/Shanghai` timezone. Neither request includes school credentials, schedules, or classroom data.
...
The BUPT service, the holiday dataset's CDN, and UAPI may process ordinary network metadata such as IP address and request time under their own policies when the app connects to them.
