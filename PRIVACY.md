...
- **Competition, summer-camp, and hackathon deadlines:** When at least one deadline category is enabled, the app downloads public event data from the Contest DDL GitHub Pages JSON endpoint over HTTPS and filters it locally to the selected date and enabled categories. If the primary endpoint is unavailable, supported clients may try the user-specified fixed backup endpoint at `http://101.201.29.29/api/contest-events`. This narrowly scoped cleartext request is a credential-free `GET` to that exact host, rejects redirects, limits response size, and never includes school credentials, cookies, tokens, schedules, classroom data, or assignment data. Each deadline category can be disabled separately in settings.
...
The BUPT service, the holiday dataset's CDN, UAPI, Timeless, GitHub Pages, and the optional Contest DDL backup service may process ordinary network metadata such as IP address and request time under their own policies when the app connects to them.
