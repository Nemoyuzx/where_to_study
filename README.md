...
| HarmonyOS | ArkTS + ArkUI（HarmonyOS NEXT 6.1.1 / API 24） | `0.2.8 (1002015)` 的 129 项单元测试、签名 APP/HAP、DevEco 上传与云测试已通过；GitHub Pre-release 提供同一修复构建 |
...
>
> HarmonyOS 测试：`0.2.8 (1002015)` 已由 DevEco 上传 AppGallery Connect 且云测试通过；[打开 0.2.8 邀请页面（链接已含邀请码，审核通过后生效）](https://appgallery.huawei.com/link/invite-test-wap?taskId=b4f098663ce7375007fb19b098feace9&invitationCode=A0IsJpKIcn3)，邀请码为 `A0IsJpKIcn3`。预审通过前公开页可能显示任务不存在；此时请从 [GitHub Pre-release](https://github.com/Nemoyuzx/where_to_study/releases/tag/v0.2.8) 获取签名 APP/HAP。

## 下载

[v0.2.8 Pre-release](https://github.com/Nemoyuzx/where_to_study/releases/tag/v0.2.8) 提供 14 项附件：Windows x64 NSIS；Linux arm64/x86_64 Debian、AppImage、CLI、TUI；固定维护者密钥签名的 Android `0.2.8 (45)` APK/AAB；HarmonyOS `1002015` 签名 APP/HAP；以及原生 macOS Universal DMG。本版本新增班车与重要事件查询、滚动增量渲染、完整本地收藏和按开关区分的日程颜色；完整中英文改动见 [0.2.8 发布说明](./docs/release-v0.2.8.md)。GitHub Release 不附带 `.sha256` 或 iOS 制品；iOS `0.2.8 (77)` 与 macOS `0.2.8 (75)` 已由本地 Xcode 上传 TestFlight，GitHub DMG 是未公证的开源预览包。因为这是 Pre-release，稳定入口 `releases/latest` 仍指向 `v0.2.7`。
