...
- 成绩：首次进入查询学校当前学期与实际学期列表，默认显示最好成绩；`t` 切换学期，
...
使用教务密码。切换账号不会复用旧账号的独立密码。TUI 当前没有作业列表；
CLI 的 `assignments` 命令可查询作业，但使用 CLI 单独保存的账户，不读取 TUI 凭据。
